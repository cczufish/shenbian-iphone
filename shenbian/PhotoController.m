...
- (void)postImageWithData:(NSDictionary *)postDict {
	CancelRequest(hcMoreInfo);
...
{    
    ...
    if (shopId && shopName && commodity && photoLinkID) {
		...
		if (nil != [PhotoController singleton].metaData) {
			CGFloat latitude = [[metaData valueForKeyPath:@"{GPS}.Latitude"] floatValue];
			CGFloat longitude = [[metaData valueForKeyPath:@"{GPS}.Longitude"] floatValue];
			NSString *dateStr = [metaData valueForKeyPath:@"{Exif}.DateTimeDigitized"];
			...
			[md setObject:[NSNumber numberWithFloat:latitude] forKey:@"x"];
			[md setObject:[NSNumber numberWithFloat:longitude] forKey:@"y"];
			[md setObject:[NSNumber numberWithInt:[date timeIntervalSince1970]] forKey:@"time"];
			
		} else if (location) {
            [md setObject:[NSNumber numberWithFloat:location.latitude] forKey:@"x"];
            [md setObject:[NSNumber numberWithFloat:location.longitude] forKey:@"y"];
        }

		postDict = md;

		DLog(@"postdict: %@", postDict);
		
		[self postImageWithData:postDict];
