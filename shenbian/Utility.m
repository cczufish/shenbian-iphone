...
+ (void)drawStars:(CGPoint)point score:(int)score
{
	...
	UIImage* starImg = [UIImage imageNamed:@"star_fill.png"];
	UIImage* starZeroImg = [UIImage imageNamed:@"star_zero.png"];
	UIImage* starHalfImg = [UIImage imageNamed:@"star_half.png"];
...
+ (void)drawVote:(NSString *)vote at:(CGPoint)point inContext:(CGContextRef)ctx
{
	if ([vote intValue] >= 1000) {
		vote = @"999+";
	}
    
    CGFloat x = 0;
    if ([vote length] > 3) { // 999+
        x = point.x - 7;
    } else if ([vote length] == 3) { // 3位数
        x = point.x - 12;
    } else { // 3位数以下
        x = point.x - 15;
    }
    
    CGRect txtRect = CGRectMake(x, point.y - 1, 25, 25);
	// thumbups 
	[[UIImage imageNamed:@"thumbup_red.png"] drawAtPoint:point];
	//text stroke
    UIColor *color       = [UIColor colorWithRed:1 green:1 blue:1 alpha:1];
    UIColor *borderColor = [UIColor colorWithRed:0.87 green:0.22 blue:0.16 alpha:1];
    [Utility drawBorderText:vote withColor:color andBorderColor:borderColor toRect:txtRect inContext:ctx];
}
